	...
	<div class="row">
				...
				<div class="ibox-content">
                            <div class="row">
                                ...
                                <select class="input-sm form-control input-s-sm inline" id="columnFiled" name="columnFiled">
                                    <option value="LIKE_name">${manager}名称</option>
                                </select>
                                ...
                                <div class="col-sm-3 text-right">
                                    ...
                                    <a href="javascript:void(0);" onclick="add();" class="btn btn-primary btn-sm">新增${manager}</a>
                                    <a href="javascript:void(0);" onclick="dels();" class="btn btn-primary btn-sm">批量删除${manager}</a>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$("#table_list").jqGrid({
					url : "${module}/${entity}/${entity}ListJson",
					datatype : "json",
	                mtype:"POST",
					height : 450,
					autowidth : true,
			...
			function add() {
				layer.open({
					...
					content : $("#popWin").load('${module}/${entity}/${entity}Add'),
				    btn: ['确定', '取消'],
			...
			function edit(id) {
				layer.open({
					...
					moveType : 1, //拖拽风格，0是默认，1是传统拖动
					shift : -1, //0-6的动画形式，-1不开启
					content : $("#popWin").load('${module}/${entity}/${entity}Edit?id=' + id),
				    btn: ['确定', '取消'],
				    yes: function(index, layero){
				    	submitForm(index);
				    	//layer.close(index); //如果设定了yes回调，需进行手工关闭
			...
			function del(id) {
				layer.confirm('确定删除该${manager}吗?', {
					btn : [ '删除', '取消' ]
				//按钮
				}, function() {
					$.post("${module}/${entity}/${entity}Del", {
						id : id
					}, function(json) {
			...
			function dels() {
				...
				layer.confirm('确定批量删除${manager}吗?', {
					btn : [ '删除', '取消' ]
				//按钮
				}, function() {
					$.post("${module}/${entity}/${entity}Dels", {
						ids : ids.join(',')
					}, function(json) {
						if (json.resultStatus == true) {//成功
							reloadGrid();
