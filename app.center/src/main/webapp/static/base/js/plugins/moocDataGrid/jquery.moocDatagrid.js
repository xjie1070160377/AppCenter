...
(function ($) {
    ...
    function request(target, param) {
        var opts = $.data(target, "moocDatagrid").options;
        if (param) {
    ...
    function getSelectRows(target) {
        var opts = $.data(target, "moocDatagrid").options;
        var data = $.data(target, "moocDatagrid").data;
        var tBody = $(target).find("tbody");
        if (opts.idField) {
            return $.data(target, "moocDatagrid").selectedRows;
        } else {
            var rowAry = [];
            $("tr.moocDatagrid-row-selected", tBody).each(function () {
                var row_index = parseInt($(this).attr("moocDatagrid-row-index"));
    ...
    function selectAll(target) {
        var opts = $.data(target, "moocDatagrid").options;
        var data = $.data(target, "moocDatagrid").data;
        var selectRows = $.data(target, "moocDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        var rows = data.rows;

        $("tr", tBody).addClass("moocDatagrid-row-selected");
        $("div.moocDatagrid-cell-check input[type=checkbox]", tBody)
            .not("div.moocDatagrid-cell-check input[disabled]").prop("checked", true);
    ...
    function clearSelectRows(target) {
        ...
        var selectRows = $.data(target, "moocDatagrid").selectedRows;
        var tBody = $(target).find("tbody");
        $("tr", tBody).removeClass("moocDatagrid-row-selected");
        $("div.moocDatagrid-cell-check input[type=checkbox]", tBody).prop("checked", false);
    ...
    function selectRow(target, index) {
        ...
        var tr = $("tr[moocDatagrid-row-index=" + index + "]", tBody);
        if (!tr.hasClass("moocDatagrid-row-selected")) {
            tr.addClass("moocDatagrid-row-selected");
            var ck = $("div.moocDatagrid-cell-check input[type=checkbox]", tr);
    ...
    function unSelectRow(target, index) {
        ...
        var tBody = $(target).find("tbody");
        var tr = $("tr[moocDatagrid-row-index=" + index + "]", tBody);
        var ck = $("tr[moocDatagrid-row-index=" + index + "] div.moocDatagrid-cell-check input[type=checkbox]", tBody);
        tr.removeClass("moocDatagrid-row-selected");
    ...
    function bindCellsEvents(target) {
        var opts = $.data(target, "moocDatagrid").options;
        var total = $.data(target, "moocDatagrid").data.total;
        ...
        $("div.moocDatagrid-cell-check input[type=checkbox]").unbind("click").bind("click", function (e) {
            var findIndex = $(this).parent().parent().parent().attr("moocDatagrid-row-index");
            if (opts.singleSelect) {
                clearSelections(target);
    ...
    function loadData(target, _data) {
        ...
        $.data(target, "moocDatagrid").data = _data;
    ...
    //插件构造函数
    $.fn.moocDatagrid = function (options, param) {
    	//如果类型为string 则调用公开方法
        if (typeof options == "string") {
            return $.fn.moocDatagrid.methods[options](this, param);
        }

        options = options || {};
        return this.each(function () {
            var state = $.data(this, "moocDatagrid");
            var opts;
            if (state) {
                opts = $.extend(state.options, options);
                state.options = opts;
            } else {
                opts = $.extend({}, $.fn.moocDatagrid.defaults, options);
                //console.log(opts);
                $.data(this, "moocDatagrid", {
                    options: opts,
                    selectedRows: [],
                    data: {
                        total: 0,
    ...
    //插件对外公开的所有方法集合
    $.fn.moocDatagrid.methods = {
        options: function (jq) {
            var opts = $.data(jq[0], "moocDatagrid").options;
            return opts;
        },
        ...
        reload: function (jq, param) {
            return jq.each(function () {
                ...
                $.data(this, "moocDatagrid").options.pageNumber = param.pageNumber;
                request(this, param);
            });
        },
        //显示正在加载提示信息
        loading: function (jq) {
            return jq.each(function () {
                var opts = $.data(this, "moocDatagrid").options;
                ...
                if (opts.loadMsg) {
                    ...
                    */
                    ...
                    if ($("#loadAjax").length === 0) {
                        html.push("<tr id='loadAjax'><td style='height:100px;text-align:center;' colspan='" + opts.columns[0].length.toString() + "'><img src='/static/base/images/dg_loading.gif'></td></tr>");
                    }
    ...
    var renderView = {
        render: function (target, container) {
            var opts = $.data(target, "moocDatagrid").options;
            var rows = $.data(target, "moocDatagrid").data.rows;
            var closFileds = $(target).moocDatagrid("getColumnFields");
            ...
            if (rows == null || rows.length <= 0) {
                ...
                var htmlAry = ["<tbody>"];
                for (var i = 0; i < rows.length; i++) {
                    var cls = (i % 2 && opts.striped) ? "class=\"moocDatagrid-row-alt\"" : "";
                    var rowStyle = opts.rowStyler ? opts.rowStyler.call(target, i, rows[i]) : "";
                    var strStyle = rowStyle ? "style=\"" + rowStyle + "\"" : "";
                    htmlAry.push("<tr moocDatagrid-row-index=\"" + i + "\" " + cls + " " + strStyle + ">");
        ...
        renderHead: function (target, container) {
            ...
            for (var i = 0; i < closFileds.length; i++) {
                ...
                var col = $(target).moocDatagrid("getColumnOption", findFiled);
            ...
            htmlAry.push("<tr id='loadAjax'><th style='height:100px;text-align:center;' colspan='" + closFileds.length.toString() + "'><img src='/static/base/images/dg_loading.gif'></th></tr>");
        ...
        renderRow: function (target, fileds, rowIndex, rowData) {
            ...
            for (var i = 0; i < fileds.length; i++) {
                ...
                var col = $(target).moocDatagrid("getColumnOption", findFiled);
                if (col) {
                    var className = col.classes ? "class=\"" + col.classes + "\"" : "";
                    ...
                    if (col.checkbox) {
                        cc.push(" class=\"moocDatagrid-cell-check ");
                    } else {
                        cc.push(" class=\"moocDatagrid-cell ");
                    }
                    cc.push("\">");
                    if (col.checkbox) {
                        if (col.formatter) {
