...
import cn.mooc.app.module.cms.orm.QuerydslUtils;

import com.mysema.commons.lang.IteratorAdapter;
import com.querydsl.core.types.EntityPath;
import com.querydsl.core.types.Predicate;
import com.querydsl.lucene3.LuceneQuery;
		...
		try {
			...
			try {
				...
				QuerydslUtils.applySorting(query, entityPath,
						pageParam.getSort());
				Integer firstResult = pageParam.getLimitStart();
				if (firstResult != null && firstResult > 0) {
					query.offset(firstResult);
				}
				Integer maxResults = pageParam.getRows();
				if (maxResults != null && maxResults > 0) {
					query.limit(maxResults);
				}
				
				return new IteratorAdapter(query.iterate()).asList();
			} finally {
				if (searcher != null) {
					searcherManager.release(searcher);
				}
		...
		try {
			...
			try {
				...
				if (total > pageable.getOffset()) {
					...
					QuerydslUtils.applySorting(query, entityPath,
							pageable.getSort());
					content = new IteratorAdapter(query.iterate()).asList();
				} else {
					content = Collections.emptyList();
				}
