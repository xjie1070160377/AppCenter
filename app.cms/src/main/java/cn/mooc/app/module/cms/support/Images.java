...
//
//import cn.csmooc.common.image.ImageInfo;
//import cn.csmooc.common.image.ScaleParam;
//import cn.csmooc.common.image.WatermarkParam;
import org.imgscalr.Scalr;
...
public class Images {
	...
	public static BufferedImage resize(BufferedImage buff, ScaleParam scaleInfo) {
		...
		if (scaleHeight == null) {
			...
			buff = Scalr.resize(buff, Scalr.Method.QUALITY, exact ? Scalr.Mode.FIT_EXACT : Scalr.Mode.AUTOMATIC, scaleWidth, scaleHeight);
		}
		return buff;
	}
	...
	public static void watermark(BufferedImage buff, BufferedImage watermark, WatermarkParam watermarkInfo) {
		watermark(buff, watermark, watermarkInfo.getPosition(), watermarkInfo.getPaddingX(), watermarkInfo.getPaddingY(), watermarkInfo.getAlphaFloat(), watermarkInfo.getMinWidth(),
				watermarkInfo.getMinHeight());
	}

	public static void watermark(BufferedImage buff, BufferedImage watermark, int postion, int paddingX, int paddingY, float alpha, int minWidth, int minHeight) {
		int width = buff.getWidth();
		int height = buff.getHeight();
		int wmWidth = watermark.getWidth();
		int wmHeight = watermark.getHeight();
		if (width < minWidth || height < minHeight || wmWidth + paddingX > width || wmHeight + paddingY > height) {
			return;
		}
		int x, y;
		switch (postion) {
